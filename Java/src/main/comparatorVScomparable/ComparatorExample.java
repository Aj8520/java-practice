package main.comparatorVScomparable;
...
public class ComparatorExample {
    public static void main(String[] args) {
        ...
        Comparator<User> compareByAgeUsingLambdas = (User user01, User user02) -> Integer.compare(user01.getAge(),
                user02.getAge());
        ...
        listOfUsers.stream().sorted((v1, v2) -> v1.getAge() - v2.getAge())
                .forEach(v -> System.out.println(v.getName()));
        System.out.println("Sorted using lambdas inside sorted() descending age : ");
        listOfUsers.stream().sorted((v1, v2) -> v2.getAge() - v1.getAge())
                .forEach(v -> System.out.println(v.getName()));

        System.out.println("Sorted using lambdas inside sorted() ascending name : ");
        listOfUsers.stream().sorted((v1, v2) -> v1.getName().compareTo(v2.getName()))
                .forEach(v -> System.out.println(v.getName()));
        System.out.println("Sorted using lambdas inside sorted() descending name : ");
        listOfUsers.stream().sorted((v1, v2) -> v2.getName().compareTo(v1.getName()))
                .forEach(v -> System.out.println(v.getName()));

        System.out.println("Sorted using CompareUsersByAgeDescendingComparator : ");
        listOfUsers.stream().sorted(new CompareUsersByAgeDescendingComparator())
                .forEach(v -> System.out.println(v.getName()));

        // if we implement comparable then only one type of sorting is allowed because
